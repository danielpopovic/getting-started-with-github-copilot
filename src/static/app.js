document.addEventListener("DOMContentLoaded", () => {
    ...
    try {
      ...
      const participantsHtml = details.participants.length > 0
        ? `<p><strong>Participants:</strong></p><ul class="participants-list">${details.participants
            .map((participant) => `<li data-email="${participant}"><span class="participant-name">${participant}</span><button type="button" class="remove-participant" aria-label="Remove participant">🗑️</button></li>`)
            .join("")}</ul>`
  ...
  activitiesList.addEventListener("click", async (event) => {
    ...
    const participantLi = button.closest("li[data-email]");
